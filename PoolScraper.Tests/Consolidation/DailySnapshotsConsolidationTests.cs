...
namespace PoolScraper.Tests.Consolidation
{
    ...
    [TestFixture]
    public class DailySnapshotsConsolidationTests
    {
        private readonly IWorkerId workerId1 = WorkerId.Create ("PoolA", 1 );
        private readonly IWorkerId workerId2 = WorkerId.Create ("PoolA", 2 );

        [Test]
        public void GetDailySnapshots_ShouldGroupSnapshotsByHourAndComputeWeightedAverage()
        {
            // Arrange
            ...
            var consolidation = new DailySnapshotsConsolidation();

            // Act
            var result = consolidation.GetDailySnapshots(snapshots).ToList();

            var day1 = result.FirstOrDefault(r => r.date == DateOnly.FromDateTime(baseDate));
            // Assert
            day1.snapshots.Should().HaveCount(1);


            // Calculate weighted average expected: (15*100 + 15*200) / (15+15) = 150
            day1.snapshots.First().BasicInfo.Hashrate.Should().Be(150);
        }
